...
<div class="colc-2-1 ">
  <a href="{{route('product.show',$catproduct->slug)}}">
   <div class="bg-gray p-2 mt-2 rd-10 " >
     ...
     <div id="imageHolder" style="height: 190px ; position: relative;" >
       ...
       bottom:0;
       left:0;
       right:0;"  src="{{asset('image/product/'.$catproduct->image)}}">
     </div>

     <div class="bg-white2 p-3 mt-2 rd-10 " >
        
       <h6 class="boldFont  " >{{Str::limit($catproduct->title,17)}}</h6>
       <p class="mp-0">4.9 | 54 Reviews</p>
       <span class="colorGreen">75% off</span> | <span class="boldFont"> BDT {{$catproduct->price}}</span>

       
       <form   action="{{route('carts.store')}}" method="post" >
         @csrf
      <input type="hidden" name="product_id" value="{{$catproduct->id}}">
